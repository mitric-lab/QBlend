...
covalentradii={"B":0.77,"C":0.77,"H":0.37,"N":0.77,"O":0.77,"Zn":1.50,"47":1.50,"19":1.50,"13":1.50,"22":1.50,"21":1.50,"30":1.50/0.529,"14":1.50/0.529, "Se":0.77, "I":0.77}
atomicradii={"H":0.7,"B":1.50,"C":1.50,"N":1.50,"Zn":2,"O":1.50,"79":2.00,"47":2.00,"19":2.00,"13":2.00,"22":2.00,"21":2.00,"14":2.00, "Se":1.5, "I": 0.7}
stickradius=0.25
...
materials={"H":makeMat("H", (0.7, 0.7, 0.7)),"B":makeMat("B", (0., 0.9, 0.)),
           ...
           "I":makeMat("I", (0.2, 0.2, 0.7)),"Se":makeMat("Se", (238.0/256, 180.0/256, 34.0/256)),
           "Au":makeMat("Au", (228.0/255.0, 202.0/255.0, 66.0/255.0)),
           "Zn":makeMat("Zn", (0.4, 0.4, 0.4)), "1.0":makeMat1("1.0", (0.8, 0., 0.)), "-1.0":makeMat1("-1.0", (0.,0.,0.8)), "0.0":makeMat1("0.0", (0.,0.,0.)), "Arrow":makeMat1("Arrow", (0., 0., 0.8))}
...
class atom(object):
    rcov = {"C": 0.77, "H": 0.37, "O":0.73, "N":0.71, "Se":0.75, "I":0.75}
    atomic_number = {"H": 1, "C": 6, "N": 7, "O": 8, "Se":34, "I":53}
    ...
    def getDistVector(self, other):
        return self.xyz.au - other.xyz.au
...
class structure:
    ...
    def readXYZ(self):
        with open(self.filename, "r") as f:
            ...
            for line in lines[2:self.natoms+2]:
                tmp = line.split()
                if len(tmp) == 4:
                    self.atomlist.append(atom(tmp[0], coord(np.array(tmp[1:4], dtype=float),
                                                            unit="angs")))
            for line in lines[self.natoms+2:]:
                self.nac.append(line.split()[:])
        self.nac = np.array(self.nac, dtype=float)   
...
class xyz2blender:
    ...
    def blenderstructure(self):
        for i in range(len(self.structure.atomlist)): 
            ...
            vec = self.structure.nac[i]  
            me = bpy.ops.mesh.primitive_cylinder_add(vertices=12, radius=stickradius*0.5,depth=np.linalg.norm(self.structure.nac[i])*10,end_fill_type="NGON")
            setMaterial(bpy.context.object, materials["Arrow"])
            bpy.ops.object.shade_smooth()
